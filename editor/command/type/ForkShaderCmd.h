...
namespace doriax::editor {
    ...
    // Forks a built-in shader and points a component's customShader at the new fork as a
    // single undoable step: execute() writes the .vert/.frag files and sets the property;
    // undo() restores the property and deletes the forked files.
    class ForkShaderCmd : public Command {
    private:
        Project* project;
        ProjectUtils::ShaderForkPlan plan;
        std::unique_ptr<Command> propertyCmd;  // sets customShader to plan.base
